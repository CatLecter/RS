...
def filter_multiple_occure(data_to_filter: list, field: str) -> list:
    ...
    while i < len(data_to_filter) - 1:
        if (
            data_to_filter[i][field] == data_to_filter[i + 1][field]
            and data_to_filter[i]["movie"]["uuid"]
            == data_to_filter[i + 1]["movie"]["uuid"]
        ):
            res = data_to_filter[i]
            while (
                res[field] == data_to_filter[i + 1][field]
                and res["movie"]["uuid"] == data_to_filter[i + 1]["movie"]["uuid"]
            ):
...
def user_prediction(user_id, model, user_id_mapping, movie_ids, movie_feature_matrix):
    """
    ...
    prediction = model.predict(
        user_ids=user_id_mapping[user_id],
...
def create_item_feature_matrix(ratings, watched, bookmarks, movie_id_mapping):
    ...
    logger.info("Количество фильмов по матрице {}".format(len(movie_id_mapping)))
    movie_genre_by_one = [
        {"movie": movie["movie"], "genre": genre, "exist": 1}
        for movie in movie_genres
        for genre in movie["genre"]
    ]
    movie_genre_exist = user_movie_columns(movie_genre_by_one, "genre", "exist")
    movie_genre_exist_pd = pd.DataFrame(
        movie_genre_exist, index=["genre", "movie", "exist"]
    ).T
...
def prediction_all(data: tuple) -> json:
    ...
    user_movie_rating = user_movie_columns(
        filter_multiple_occure(ratings, "user_uuid"), "user_uuid", "rating"
    )
    ...
    movie_feature_matrix = create_item_feature_matrix(
        ratings, watched, bookmarks, movie_id_mapping
    )

    # Создаем модель
    model = LightFM(loss="warp")
    logger.info(
        "Параметры модели: размеры матрицы {} на {}".format(
            len(user_id_mapping), len(movie_id_mapping)
        )
    )
    model.fit(
        interactions=user_movie_rating_matrix,
        item_features=movie_feature_matrix,
    ...
    for user in user_id_mapping:
        res = user_prediction(
            user, model, user_id_mapping, movie_ids, movie_feature_matrix
        )
